...
class Manager(manager.Manager):
    ...
    def remove_user_from_group(self, user_id, group_id, domain_scope=None):
        ...
        #TODO(ayoung) revoking all tokens for a user based on group membership
        #is Overkill, as we only would need to revoke tokens that had role
        #assignments via the group.  Calculating those assingments would
        #have to be done by the assignment backend.
        self.revoke_api.revoke_by_user(user_id)
        self.token_api.delete_tokens_for_user(user_id)
