...
class Manager(manager.Manager):
    ...
    def get_inferred_roles(self, prior_role_id):
        return self.role_api.list_inferred_roles(prior_role_id)

    def _add_inferred_roles(self, roles):
        ...
        while(roles_to_check):
            ...
            if (next_role in checked_roles):
                continue
            inferred_roles = self.get_inferred_roles(next_role)
            for inferred_role in inferred_roles:
                inferred_id = inferred_role['inferred_role_id']
                if inferred_id in checked_roles:
                    continue
                roles.append(inferred_id)
                roles_to_check.append(inferred_id)
        return roles
    ...
    def get_roles_for_user_and_project(self, user_id, tenant_id):
        ...
        roles = self._add_inferred_roles(user_role_list + group_role_list)
...
class RoleManager(manager.Manager):
    ...
    @abc.abstractmethod
    def list_inferred_roles(self, prior_id):
        return self.driver.list_inferred_roles(prior_id)
...
class RoleDriverV8(object):
    ...
    @abc.abstractmethod
    def create_inferred_role(self, proir_role_id, inferred_role_id):
    ...
    @abc.abstractmethod
    def delete_inferred_role(self, proir_role_id, inferred_role_id):
    ...
    @abc.abstractmethod
    def list_inferred_roles(self, prior_id):
        """Lists roles inferred from the prior id
