...
class Role(assignment.RoleDriverV8):
    ...
    @sql.handle_conflicts(conflict_type='inferred_role')
    def create_inferred_role(self, prior_role_id, inferred_role_id):
        with sql.transaction() as session:
            inference = {'prior_role_id': prior_role_id,
                         'inferred_role_id': inferred_role_id
                         }
            ref = InferredRoleTable.from_dict(
                inference, include_extra_dict=False)
            session.add(ref)
            return ref.to_dict(include_extra_dict=False)

    @sql.handle_conflicts(conflict_type='inferred_role')
    def delete_inferred_role(self, prior_role_id, inferred_role_id):
        """Deletes are role inference rule
        :raises: keystone.exception.RoleNotFound

        """
        with sql.transaction() as session:
            query = session.query(InferredRoleTable)
            query.filter(and_(
                InferredRoleTable.prior_role_id == prior_role_id,
                InferredRoleTable.inferred_role_id == inferred_role_id))
            refs = query.all()
            for ref in refs:
                ...
                if (ref.prior_role_id == prior_role_id
                        and ref.inferred_role_id == inferred_role_id):
                    session.delete(ref)

    @sql.handle_conflicts(conflict_type='inferred_role')
    def list_inferred_roles(self, prior_role_id):
        with sql.transaction() as session:
            query = session.query(
                InferredRoleTable).filter(
                    InferredRoleTable.prior_role_id == prior_role_id)
            refs = query.all()
            return [ref.to_dict(include_extra_dict=False) for ref in refs]


class InferredRoleTable(sql.ModelBase, sql.DictBase):
    ...
    __tablename__ = 'inferred_role'
    attributes = ['prior_role_id', 'inferred_role_id']
    prior_role_id = sql.Column(sql.String(64), primary_key=True)
    inferred_role_id = sql.Column(sql.String(64), primary_key=True)
    __table_args__ = (
        sql.UniqueConstraint('prior_role_id', 'inferred_role_id'), {})
