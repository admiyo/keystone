...
class InferredRoleTests(test_v3.RestfulTestCase,
                        unit.TestCase):
    ...
    def test_list_inferred_roles_none(self):
        prior = self._create_role()
        url = '/inferred_roles/%s' % (prior['id'])
        r = self.get(url)
        self.assertEqual(r.result['prior_role_id'], prior['id'])
        self.assertEqual(0, len(r.result['inferred_roles']))

    def _create_inferred_role(self, prior, inferred):
        url = '/inferred_role/%s/%s' % (prior['id'], inferred['id'])
        self.put(url)

    def _delete_inferred_role(self, prior, inferred):
        url = '/inferred_role/%s/%s' % (prior['id'], inferred['id'])
        self.delete(url)

    def test_CRUD_inferred_role(self):
        prior = self._create_role()
        inferred1 = self._create_role()
        self._create_inferred_role(prior, inferred1)
        inferred2 = self._create_role()
        self._create_inferred_role(prior, inferred2)

        r = self.get('/inferred_roles/%s' % (prior['id']))

        self.assertEqual(r.result['prior_role_id'], prior['id'])
        self.assertIn(inferred1['id'], r.result['inferred_roles'])
        self.assertIn(inferred2['id'], r.result['inferred_roles'])
        self.assertEqual(2, len(r.result['inferred_roles']))

        self._delete_inferred_role(prior, inferred2)
        r = self.get('/inferred_roles/%s' % (prior['id']))

        self.assertEqual(r.result['prior_role_id'], prior['id'])
        self.assertIn(inferred1['id'], r.result['inferred_roles'])
        self.assertNotIn(inferred2['id'], r.result['inferred_roles'])
        self.assertEqual(1, len(r.result['inferred_roles']))
